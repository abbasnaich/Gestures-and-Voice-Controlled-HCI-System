...
class CinematicHUD:
    ...
    def draw(self, img, hand_landmarks, state, confidence, is_paint_active=False):
        """
        Draw the HUD.
        :param img: Canvas
        ...
        :param is_paint_active: Whether we are in Paint Writing Mode
        """
        if not hand_landmarks:
            self.trail.clear()
            # Draw Mode info even if no hand
            if is_paint_active:
                cv2.putText(img, "PEN MODE", (20, 70), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 165, 255), 2)
            return
